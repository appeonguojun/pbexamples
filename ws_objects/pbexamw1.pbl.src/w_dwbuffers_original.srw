...
$PBExportComments$データウィンドウ バッファ サンプル プログラムのオリジナル バッファを表示するシート
forward
...
If ls_Ret <> "" Then
	MessageBox("エラー", "カラム 1 の更新に失敗しました。", Exclamation!)
End If 

ls_ModString = "#2.Color='0~tif(#4=~~~"delete~~~",255,if(#4=~~~"filter~~~",65280,0))'"
ls_Ret = dw_data.Modify(ls_ModString)
If ls_Ret <> "" Then
	MessageBox("エラー", "カラム 2 の更新に失敗しました。", Exclamation!)
End If 

ls_ModString = "#3.Color='0~tif(#4=~~~"delete~~~",255,if(#4=~~~"filter~~~",65280,0))'"
ls_Ret = dw_data.Modify(ls_ModString)
If ls_Ret <> "" Then
	MessageBox("エラー", "カラム 3 の更新に失敗しました。", Exclamation!)
End If 
