...
$PBExportComments$SetTrans 関数または SetTransObject 関数でデータウィンドウを使った例
forward
...
st_settransobject.text = "SetTrans 関数はデータウィンドウが内部で用いるトランザクション オブジェクトを"+ &
"使う場合に指定します。 SetTrans 関数を用いると、Retrieve や  Update などの"+ &
"データウィンドウ関数が実行される度にデータベースとの接続とその解除が行われ、"+ &
"実行が遅くなりますがデータベース ロックを短時間にとどめることができます。"
...
st_settransobject.Text =  "SetTransObject 関数は、ユーザがデータウィンドウのトランザクション管理を行いたいときに用います。" + &
" データウィンドウ関数は実行時にデータベースとの接続を必要とせず、速く実行することができます。" + &
" SetTransObject 関数を用いると、埋め込みSQL にも同じトランザクションを用いることができますが、" + &
"トランザクション管理を自分で行う必要があります。"
...
If SQLCA.SQLCode <> 0 then MessageBox("SQLエラー",SQLCA.SQLErrText,Exclamation!)
end on
...
// Error handling
If li_rtn = -1 Then
	MessageBox ('データウィンドウ更新エラー', SQLCA.SQLErrText, exclamation!)
end if
...
If rb_settrans.checked = True then
	st_st_time.text = string((ll_after_time - ll_before_time)/1000) + " 秒"
Else
	st_sto_time.text = string((ll_after_time - ll_before_time)/1000) + " 秒"
End If
...
If SQLCA.SQLCode <> 0 then MessageBox("SQL エラー",SQLCA.SQLErrText,Exclamation!)
