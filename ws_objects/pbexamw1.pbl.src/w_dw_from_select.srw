...
$PBExportComments$ユーザが選択したテーブルとカラムを元にデータウィンドウを作成します。
forward
...
integer width = 3152
integer height = 1995
string title = "SQL または dwSyntax によるデータウィンドウ生成"
...
if f_set_table_select(dw_tables) < 0 then
	MessageBox("エラー", &
					"テーブルリストに対する SQL Select ステートメントを設定できませんでした", &
					StopSign!)
...
If f_set_column_select(dw_columns,is_table) < 0 Then
	MessageBox("エラー", &
					"カラムリストに対する SQL Select ステートメントを設定できませんでした。", &
					StopSign!)
	Return
End If
...
// something there.
if mle_1.text = "" then
		MessageBox("エラー", "データウィンドウに SQL 文が存在しません。適切な SQL 文または" &
	+ "データウィンドウ構文を入力するか、テーブルとカラムを選択して <データウィンドウの作成> " &
	+ "ボタンをクリックしてください。")
	Return
end if
