...
$PBExportComments$DDE サーバーが w_dde_client とどのように機能するかデモンストレーションする
forward
...
if status.checked then
	...
	if resp <> 1 then messagebox("ホットリンク ハンドル エラー","データ エリア エラー = "+item+". エラー コード = "+string(resp))
end if
end subroutine
...
set_msg("Remoterequest 始動 ~r~n  アプリケーション = "+appl+"~r~n  トピック = "+topic+"~r~n  リクエスト = "+data)
choose case data
	case is_item1
		data = sle_data1.text
	case is_item2
...
// what is the command
getcommanddde(data)
Set_msg("remoteexec が始動, ~r~n  アプリケーション = "+appl+"~r~n  リクエスト = "+data)
...
Set_msg("remotehotlinkstart 始動 ~r~n  アプリケーション = "+appl+"~r~n  トピック = "+topic+"~r~n  アイテム = "+item)
choose case item
	case is_item1
	...
	case else
		messagebox("remotehotlinkstart","未知なアイテムがリクエストされました。 アイテム = "+item)
		respondremote(false)
		return
end choose
...
getdataddeorigin(appl,topic,item)
Set_msg("remotehotlinkstop 始動 ~r~n  アプリケーション = "+appl+"~r~n  トピック = "+topic+"~r~n  アイテム = "+item)
choose case item
	case is_item1
