...
$PBExportComments$コール トラックのサンプル プログラム。ID カラムのデフォルトに自動インクリメントを使用。
forward
...
integer width = 2949
integer height = 1852
string title = "AutoIncrement - 通話記録"
...
event clicked;If ib_Changed Then
	If MessageBox("新規追加", "現在の電話履歴を保存しますか?", Question!, &
...
event clicked;If row > 0 Then
	If ib_Changed Then
		If MessageBox("電話履歴の更新", "変更を更新しますか?", Question!, &
								YesNo!) = 1 Then cb_update.Trigger Event Clicked()
	End If
