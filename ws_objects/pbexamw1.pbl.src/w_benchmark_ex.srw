...
$PBExportComments$パフォーマンス ベンチマーク サンプル プログラム
forward
...
integer width = 1751
integer height = 344
string title = "PowerBuilder 5.0 ベンチマーク比較"
...
event open;call super::open;this.Title = "PowerBuilder " + sVersion + " ベンチマーク比較"

dw_1.Object.t_14.Text = "PowerBuilder " + SVersion + " ベンチマーク"
end event
...
string text = "名前を付けて保存..."
end type

event clicked;String	ls_File, ls_Path, ls_Ext

GetFileSaveName("レポートの保存", ls_Path, ls_File, "psr", "PSR ファイル (*.psr),*.psr, " + &
							"テキスト ファイル(*.txt),*.txt, HTML ファイル (*.htm),*.htm")
...
If Trim(ls_Path) <> "" Then
	...
	Choose Case ls_Ext
		...
		Case Else
			MessageBox("レポートの保存", "次のファイル 形式で保存して下さい。 " + &
								"PBレポート ファイル - '.psr', テキスト ファイル - '.txt'または HTML ファイル - '.htm'", Exclamation!, OK!)
			Return
	End Choose
End if
end event
...
If of_check_vals("Benchmark P-Code Tests", "loop", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report + "ループ処理~t空の For/Next ループを実行します。~t" + &
...
If of_check_vals("Benchmark P-Code Tests", "tak", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report + "TAK ベンチマーク~tTAK ベンチマークは比較ツールで一般的に関数呼び出しで利用されるアルゴリズムです。~t" + &
					ls_PNum + "~t" + ls_PTime +"~t" + ls_MNum + "~t" + ls_MTime +"~r~n"
End If
...
If of_check_vals("Benchmark P-Code Tests", "sort", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report + "配列ソート~t配列の値を初期と逆の順番に並び替えます。~t" + &
					ls_PNum + "~t" + ls_PTime +"~t" + ls_MNum + "~t" + ls_MTime +"~r~n"
End If
...
If of_check_vals("Benchmark P-Code Tests", "function", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report + "グローバル関数のオーバーヘッド~t定義のみで処理の記述がないグローバル関数を~n~r呼び出します。~t"  + &
					ls_PNum + "~t" + ls_PTime + "~t" + ls_MNum + "~t" + ls_MTime +"~r~n"
End If
...
If of_check_vals("Benchmark P-Code Tests", "ofunction", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report +  "オブジェクト関数のオーバーヘッド~t定義のみの空のオブジェクト (ウィンドウ) 関数を~n~r呼び出します。~t" + &
					ls_PNum + "~t" + ls_PTime +"~t" + ls_MNum + "~t" + ls_MTime +"~r~n"
End If
...
If of_check_vals("Benchmark P-Code Tests", "event", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report + "イベント オーバーヘッド~t定義のみで処理の記述がないイベントを起動します。~t" + &
					ls_PNum + "~t" + ls_PTime +"~t" + ls_MNum + "~t" + ls_MTime +"~r~n"
End If
...
If of_check_vals("Benchmark P-Code Tests", "window", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report + "ウィンドウのインスタンスの作成~tコントロールを保持しないウィンドウを開きます。~t" + &
					ls_PNum + "~t" + ls_PTime + "~t" + ls_MNum + "~t" + ls_MTime +"~r~n"
End If
...
If of_check_vals("Benchmark P-Code Tests", "string", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report + "文字列の置換~tある文字列をすべて、別の文字列で"  + &
									"within 置き換えます。~t" + &
					ls_PNum + "~t" + ls_PTime +"~t" + ls_MNum + "~t" + ls_MTime +"~r~n"
End If
...
If of_check_vals("Benchmark P-Code Tests", "var", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report + "グローバル変数~tグローバル変数に値を割り当てます。~~t" + &
					ls_PNum + "~t" + ls_PTime +"~t" + ls_MNum + "~t" + ls_MTime +"~r~n"
End If
...
If of_check_vals("Benchmark P-Code Tests", "const", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report + "グローバル定数~tグローバル変数に定数を割り当てます。~t" + &
					ls_PNum + "~t" + ls_PTime +"~t" + ls_MNum + "~t" + ls_MTime +"~r~n"
End If
...
If of_check_vals("Benchmark P-Code Tests", "get", ls_PNum, ls_PTime) And &
	...
	ls_Report = ls_Report + "データストアを使用した場合~tGetItem 関数を使用して、データストアから変数に~n~rすべての行をコピーします。" + &
									"次に SetItem 関数を使用して、別のデータストアに書き込みます。~t" + &
					ls_PNum + "~t" + ls_PTime +"~t" + ls_MNum + "~t" + ls_MTime +"~r~n"
End If
...
If ls_Path = "" Then
	If MessageBox ( "PB Runtime", "PB ランタイムがみつかりません。" + &
								"~n~r~n~r次の画面でファイルを選択し、ランタイムディレクトリを指定してください。 " + &
								"~n~r~n~rすぐに指定しますか？", Exclamation!, YesNo!, 2) = 2 then
		Return
	Else
		// Allow the user to find the directory
		if GetFileOpenName("PB ランタイム ディレクトリのファイルを選択してください。", ls_Path, ls_file ) = 1 then
...
If Run(ls_exe) = -1 Then
	MessageBox("エラー", "マシンコードベンチマークの実行中にエラーが発生しました (exbm_m.exe)", &
						StopSign!)
End If
end event
...
If Run(ls_exe) = -1 Then
	MessageBox("エラー", "P-Code ベンチマークの実行中にエラーが発生しました (exbm_m.exe)", &
						StopSign!)
End If
end event
