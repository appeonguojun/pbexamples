...
$PBExportComments$データウィンドウに適合する構造体を作成するユーティリティ。
forward
...
integer width = 1928
integer height = 1444
string title = "構造体とデータウィンドウ間のコピー"
...
if sle_filename.text = "" then
	messagebox("不適切なファイル名", "ファイル名を入力してください。", Exclamation!)
	return -1
end if
...
if ids_create.create(ls_syntax) < 0 then
	messagebox("SQL 構文エラー", "データウィンドウ生成中にエラーが発生しました。")
	return -1
end if
...
GetFileOpenName ( "ライブラリの選択", ls_path, ls_file, "PBL",&
"ライブラリ ファイル (*.PBL),*.PBL")  
