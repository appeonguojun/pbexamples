...
$PBExportComments$データウィンドウ バッファ サンプル プログラムのプライマリ バッファを表示するシート
forward
...
event ue_undo;call w_dwbuffers_ancestor::ue_undo;
If MessageBox("変更のロールバック",  "データベースへの変更をロールバックしてもよろしいですか？", &
...
If m_dwbuffers.m_options.m_showsqlpreview.Checked Then
	...
	ls_RowBufferTitle = "SQL プレビュー:  行: " + String (row) + "  バッファ:  " + ls_BufferUsed
