...
$PBExportComments$DDE サーバーのデモ(w_dde_server)
forward
...
respondremote(true)
set_msg("Hotlinkalarm が起動されました。 ~r~n  アプリケーション = "+appl+" トピック= "+topic+"~r~n  アイテム = "+item+"~r~n  データ = "+data)
end on
...
if resp <> 1 then 
	messagebox("ホットリンクの停止=","エラー コード = "+string(resp))
	return
end if
...
if resp <> 1 then 
	messagebox("ホットリンク エラー",string(resp))
	return
end if
...
if resp = 1 then 
	...
	messagebox("getremote エラー",String(resp))
end if
