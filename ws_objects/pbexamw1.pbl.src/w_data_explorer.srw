...
$PBExportComments$データ エクスプローラ (TreeView と ListView)
forward
...
For li_Cnt = 1 To ai_Rows
	...
	If tv_1.InsertItemLast(al_Parent, ltvi_New) < 1 Then
		// Error
		MessageBox("エラー", "項目の挿入でエラーが発生しました", Exclamation!)
		Return -1
	End If
Next
...
Choose Case ai_Level
	Case 2
		lv_1.AddColumn("従業員氏名", Left!, 500)
		lv_1.AddColumn("従業員ID", Left!, 150)
		lv_1.AddColumn("部署ID", Left!, 160)
		lv_1.AddColumn("管理者", Left!, 250)
		lv_1.AddColumn("勤務状況", Center!, 195)
		lv_1.AddColumn("電話番号", Left!, 350)
		lv_1.AddColumn("給与", Right!, 280)
		ii_Columns = 7
	Case 3
		lv_1.AddColumn("会社名", Left!, 700)
		lv_1.AddColumn("会社ID", Left!, 150)
		lv_1.AddColumn("担当者氏名", Left!, 500)
		lv_1.AddColumn("住所", Left!, 1150)
		lv_1.AddColumn("電話番号", Left!, 350)
		ii_Columns = 5
	Case 4
		lv_1.AddColumn("注文ID", Left!, 250)
		lv_1.AddColumn("注文日", Left!, 250)
		lv_1.AddColumn("会計コード", Center!, 375)
		lv_1.AddColumn("地域", Left!, 250)
		ii_Columns = 4
	Case 5
		lv_1.AddColumn("商品名", Left!, 500)
		lv_1.AddColumn("項目番号", Right!, 195)
		lv_1.AddColumn("商品ID", Left!, 150)
		lv_1.AddColumn("出荷日", Left!, 275)
		lv_1.AddColumn("数量", Right!, 150)
		lv_1.AddColumn("値段", Right!, 270)
		lv_1.AddColumn("合計", Right!, 250)
		ii_Columns = 7
End Choose
...
For li_Cnt = 1 To ai_Rows
	...
	If lv_1.AddItem(llvi_New) < 1 Then
		// Error
		MessageBox("エラー", "リストビューへの項目追加でエラーが発生しました。", Exclamation!)
		Return -1
	End If
Next
...
Choose Case ai_Level
	Case 2
		...
		alvi_New.Label = alvi_New.Label + String(ldb_Number, "[currency]")
	...
	Case 5
		...
		alvi_New.Label = alvi_New.Label + String(ldb_Number, "[currency]") + "~t"
