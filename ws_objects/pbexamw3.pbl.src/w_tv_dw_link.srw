...
$PBExportComments$ツリービューとデータウィンドウのリンクを示すウィンドウです。
forward
...
integer width = 2932
integer height = 1835
string title = "ツリービューとデータウィンドウのリンク"
...
For li_Cnt = 1 To ai_Rows
	...
	If tv_1.InsertItemLast(al_Parent, ltvi_New) < 1 Then
		// Error
		MessageBox("エラー", "項目の追加でエラーが発生しました。", Exclamation!)
		Return -1
	End If
Next
...
li_Ret = MessageBox("警告", "ツリービューの別の項目を選択すると、変更が破棄されます。変更を保存しますか？", Question!, YesNoCancel!)
