...
$PBExportComments$Quick Select レスポンス ダイアログ, クェリー ソートの例
forward
...
if f_set_table_select(dw_tables) < 0 then
	MessageBox("エラー", &
					"選択したテーブルに対する Select ステートメントを設定できませんでした", &
					StopSign!)
...
//**********************************************************************
	...
	If Trim(lComments) = "" or IsNull(lComments)  then
		mle_comments.text = "このカラムにはコメントがありません。"
	else
...
dw_style =	'style(Type=grid) ' + &
				'Text(font.face="ＭＳ Ｐゴシック"  font.Height=12  font.weight=400 font.family=1' + &
						'font.pitch=1 font.charset=-128) ' + &
				'Column(font.face="ＭＳ Ｐゴシック"  font.Height=12  font.weight=400 font.family=1' + &
						'font.pitch=1 font.charset=-128) ' 
...
mle_comments.Text = '~t ' + Lower ( ls_tname ) + ' テーブルのカラムを検索しています...'
...
If f_set_column_select(dw_columns, is_TableName) < 0 Then
	MessageBox("エラー", &
					"選択したカラムに対する Select ステートメントを設定できませんでした", &
					StopSign!)
	Return
End If
...
If Len(ls_errors) = 0 Then
	...
	If dw_criteria.Create (ls_dw_syntax) > 0 Then
		...
		For li_index = 1 to li_num_of_gobs
			...
			ls_hdgs[li_index] + '.Font.Face="ＭＳ Ｐゴシック" ' + &
			ls_hdgs[li_index] + '.Font.Height="60" ' + &
			ls_hdgs[li_index] + '.Height="60" ' + &
			ls_hdgs[li_index] + '.Font.Weight="400" ' + &
			ls_hdgs[li_index] + '.Font.Family="1" ' + &
			ls_hdgs[li_index] + '.Font.Pitch="2" ' + &
			ls_hdgs[li_index] + '.Font.Charset="-128" ' + &
			ls_cols[li_index]  + '.y="4" ' + &
			ls_cols[li_index] + '.Font.Face="ＭＳ Ｐゴシック" ' + &
			ls_cols[li_index] + '.Font.Height="52" ' + &
			ls_cols[li_index] + '.Height="60" ' + &
			ls_cols[li_index] + '.Font.Weight="400" ' + &
			ls_cols[li_index] + '.Font.Family="1" ' + &
			ls_cols[li_index] + '.Font.Pitch="2" ' + &
			ls_cols[li_index] + '.Font.Charset="-128" ' + &
			ls_cols[li_index] + '.Visible="0" ' + &
			' '
			
			if dw_criteria.describe(ls_cols[li_index] + ".Edit.Style") = "radiobuttons" then
				ls_values = dw_criteria.Describe(ls_cols[li_index] + ".Values")
				li_count = 0
				li_start = pos(ls_values, "/")
				DO UNTIL li_start = 0
					li_count++
					li_start = pos(ls_values, "/", li_start + 1)
				LOOP
				ls_name += ls_cols[li_index] + '.RadioButtons.Columns="' + string(li_count) + '" '
			end if
...
//**********************************************************************
	...
	If Trim(ls_comments) = "" or IsNull(ls_comments)  then
		mle_comments.text = "このテーブルにはコメントがありません。"
	else
...
long backcolor = 74481808
boolean enabled = false
string text = "テーブルやカラムのコメントが表示されます。"
boolean focusrectangle = false
end type
...
long backcolor = 74481808
boolean enabled = false
string text = "3. (任意) 選択されたカラム ボックスに"
boolean focusrectangle = false
end type
