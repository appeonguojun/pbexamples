...
$PBExportComments$ソートとフィルタの指定ができるさまざまなデータウィンドウ関数
forward
...
If SQLCA.SQLCode = -1 then
	MessageBox ('エラー', SQLCA.SQLErrText, exclamation!)
End If
...
this.Control[iCurrent+3]=this.ddlb_data
this.Control[iCurrent+4]=this.cb_filter
this.Control[iCurrent+5]=this.cb_close
this.Control[iCurrent+6]=this.ddlb_filtercol
...
this.Control[iCurrent+13]=this.ddlb_sortcol
this.Control[iCurrent+14]=this.rb_desc
this.Control[iCurrent+15]=this.rb_asc
...
string text = "データウィンドウの変更にソートおよびフィルタオプションを使用してください。データウィンドウの基本的な機能を使用するには、画面右のボタンを使用してください。"
...
If cb_filter.text = "フィルタ解除(&U)" Then wf_filter_dw()
end event
...
If cb_filter.Text = "フィルタ解除(&F)" then
	cb_filter.Text = "フィルタ(&F)"
	filt=""
	dw_1.Setfilter(filt)
	dw_1.filter( )
Else
	cb_filter.Text = "フィルタ解除(&F)"
	wf_filter_dw()
End If
end event
...
If cb_filter.text = "フィルタ解除(&F)" Then cb_filter.Triggerevent(clicked!)
