...
$PBExportComments$新規作成されたデータウィンドウを持つシート
forward
...
integer Width=2862
integer Height=1664
string title="ランタイムデータウィンドウペインタ"
...
If Handle(w_quick_select) < 1 then
	MessageBox("クイックセレクトによる選択がありません", "データウィンドウを生成できませんでした")
	Return
else	
	//*********************************************************************
	...
	ls_style = 'style(type=' + ls_report_type + ')' + &
				...
				'font.face = "ＭＳ Ｐゴシック"  font.height = -12  font.weight = 400 font.family = 1' + &
						'font.pitch=1   font.charset="-128") ' + &
				'Column(background.mode=0 background.color=1073741824 color=0 ' +&
				'font.face = "ＭＳ Ｐゴシック"  font.height = -10  font.weight = 400 font.family = 1' + &
						'font.pitch = 1 font.charset="-128" ) ' 
	this.title = ls_tablename + " テーブルのデータウィンドウ"  
...
Else
	messagebox("データウィンドウの作成エラー",  "(戻り値) "+ string(li_createreturncode))
...
If ls_rc <> "" then messagebox("Modify エラー",ls_rc)
end on
