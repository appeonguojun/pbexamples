...
$PBExportComments$行単位とページ単位でスクロールするスクロールユニット
forward
...
event modified;// change the window attribute UnitsPerLine, which specifies the 
// number of PowerBuilder units to scroll when the arrow of the vertical
// scroll bar is clicked.

Int li_cnt

li_cnt = Integer (this.text)
parent.UnitsPerLine = li_cnt
if li_cnt > 1 then
	parent.vscrollbar = true
end if
