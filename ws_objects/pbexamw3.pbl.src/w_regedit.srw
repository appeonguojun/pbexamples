...
lv_vals.AddColumn("名前", Left!, 600)
lv_vals.AddColumn("データ", Left!, 1500)
...
If (ge_Environment.OSType = Windowsnt!) Or &
	...
	Do
		...
		If Lower(ltvi_Item.Label) <> "sybase" Then
			ll_Handle = tv_reg.FindItem(NextTreeItem!, ll_Handle)
		End if
	Loop Until Lower(ltvi_Item.Label) = "sybase" OR ll_Handle < 0
...
// label edit mode to allow the user to change it.
li_Index = lv_vals.AddItem("新しい値" + "~t~"" + "~"", 0)
lv_vals.SetFocus()
...
If MessageBox("エントリーの削除", llvi_Item.Label + " に対するレジストリエントリを削除しますか？", &
...
If MessageBox("キーの削除", "レジストリキー " + ls_Key + " を削除しますか？", &
						Question!, YesNo!, 2) = 2 Then Return
