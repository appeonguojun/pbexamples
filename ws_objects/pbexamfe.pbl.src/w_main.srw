...
$PBExportComments$Code Examples フロント エンド インタフェース
forward
...
tab_examples.tabpage_search.gb_search.x = tab_examples.width - 580
...
string picturename[] = {"Application!","DataPipeline!","Query5!","DataWindow5!","DataWindow5!","DataWindow5!","DataWindow5!","DataWindow5!","Move5!","Function!","Function!","Function!","Graph!","Inherit!","Custom002!","Cascade!","Menu5!","Custom045!","Custom020!","Ole5!","Continue!","PasteSQL5!","PasteSQL5!","UserObject5!","Window!","Run!","Custom079!","CreateLibrary!","exam.bmp"}
long picturemaskcolor = 12632256
...
ltvi_Item.Label = "PowerBuilder サンプル"
ltvi_Item.Data = ""
...
For li_Cnt = 1 To li_Categories
	...
	if li_Sub > 0 Then
		...
		If ll_Super = 0 Then
			...
			ltvi_Item.PictureIndex = li_Cnt + 1
			ltvi_Item.SelectedPictureIndex = ltvi_Item.PictureIndex
			ltvi_Item.Children = True
...
boolean vscrollbar = true
string item[] = {"すべての機能","5.0からの新機能","5.0からの拡張","8.0以降の新機能"}
borderstyle borderstyle = stylelowered!
		...
		if li_Sub > 0 Then
			...
			If ll_Super = 0 Then
				...
				If of_has_children(ids_SearchExamples, ls_Super) Then
					ltvi_Item.Label = ls_Super
					ltvi_Item.Data = ls_Super
					ltvi_Item.PictureIndex = li_Pict + 1
					ltvi_Item.SelectedPictureIndex = ltvi_Item.PictureIndex
					ltvi_Item.Children = True
...
long textcolor = 33554432
long backcolor = 73955432
string text = "PowerScript 関数(&P)"
end type

event clicked;Post of_change_object('psfunc','PowerScript 関数')
end event
...
long textcolor = 33554432
long backcolor = 73955432
string text = "ユーザ定義関数(&U)"
end type

event clicked;Post of_change_object('userfunc','ユーザ定義関数')
end event
...
long textcolor = 33554432
long backcolor = 73955432
string text = "オブジェクト(&O)"
end type

event clicked;Post of_change_object('object','オブジェクト')
end event
...
If ls_OrigSQL = is_search_sql Then
	MessageBox('検索','１つ以上の検索項目を選択してください。')
	Return
End If
...
event clicked;dw_search.DataObject = dw_search.DataObject
dw_search.SetTransObject(sqlca)
of_start_querymode(True)
dw_search.SetRedraw(False)

If rb_objects.Checked Then 
	of_change_object('object','オブジェクト')
Elseif rb_powerscript.Checked Then
	of_change_object('psfunc','PowerScript 関数')
Elseif  rb_user_func.Checked Then
	of_change_object('userfunc','ユーザ定義関数')
End If
