...
$PBExportComments$従業員情報システムの従業員詳細を表示
forward
...
if istr_info.employee_id = 0 then
	...
	if	dw_empl.Retrieve (istr_info.employee_id) <= 0 then
		...
		this.title = "従業員詳細 - " + ls_emp_lname + " " + ls_emp_fname
	end if
end if
end event
...
if dw_empl.RowCount() > 0 then
	...
	if l_mod = DataModified! or l_mod = NewModified! then
		...
		if MessageBox ('従業員詳細', 'ウィンドウを閉じる前に変更を保存しますか？', &
...
ls_emp_fname = dw_empl.Object.emp_fname[1]
ls_emp_lname = dw_empl.Object.emp_lname[1]
ls_empname = ls_emp_lname + ' ' + ls_emp_fname

// Ask if it's ok to delete
if MessageBox ('従業員情報システム', '従業員 ' + &
					ls_empname + ' を削除しますか？', question!, YesNo!) = 1 then
	dw_empl.DeleteRow (0)
	wf_update (false)
	close (parent)
end if
