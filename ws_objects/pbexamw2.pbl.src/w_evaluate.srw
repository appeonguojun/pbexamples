...
$PBExportComments$ItemError イベントと describe 関数を用いて DataWindow のフィールドの内容を同じか評価する
forward
...
string Title="Evaluate関数とItemErrorイベント"
...
If this.GetColumnName() = 'employee_salary' Then
	...
	If isNumber(ls_result) Then
		...
		this.object.employee_salary[row] = dec(ls_result)
