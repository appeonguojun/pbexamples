...
$PBExportComments$GetItemStatus 関数とSetItemStatus 関数が更新にどうのような影響を及ぼすか表わすサンプル
forward
...
ll_row = dw_1.GetRow()
if ll_row = 0 then
	MessageBox ("エラー", "当該行が存在しません。項目ステータスは変更されませんでした。", exclamation!)
	return
end if
ls_col = dw_1.GetColumnName()
if ls_col = "" then
	MessageBox ("エラー", "当該カラムが存在しません。項目ステータスは変更されませんでした。", exclamation!)
	return
end if
...
if it_trans.SQLCode = -1 then
	MessageBox("DSN接続エラー: 'PB Demo DB V2017R2'", it_trans.SQLErrText, exclamation!)
end if
...
st_label.text = ls_col_label + "のステータス :"
...
long backcolor = 74481808
boolean enabled = false
string text = "Sales Order ID カラムステータス:"
boolean focusrectangle = false
end type
...
if ib_update then
	...
	OpenWithParm (w_step, "更新関数@" + sqlsyntax + "により、SQL構文が出力されました")		
