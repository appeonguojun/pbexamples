...
$PBExportComments$メイン メモ帳 ウィンドウ
forward
...
if ib_text_modified then
	if MessageBox ("PowerBuilt メモ帳", "保存しますか?", &
...
if GetFileOpenName ("ファイルを開く", is_fullname, is_filename, "txt", &
							"テキスト ファイル (*.txt),*.txt,INI Files (*.ini)," + &
							"*.ini,バッチ ファイル (*.bat),*.bat") < 1 then return
...
//////////////////////////////////////////////////////////////////////
if Left (is_filename, 8) = "文書" and Match (Mid (is_filename, 9), "[0-9]")  then
	ls_save_filename = ""
	ls_save_pathname = ""
else
...
if GetFileSaveName ("ファイルの保存", ls_save_pathname, &
...
if ls_save_pathname <> ls_old_fullname then
	if FileExists (ls_save_pathname) then
		li_ret = Messagebox ("PowerBuilt メモ帳", &
							ls_save_pathname + " を上書き保存しますか?", &
...
// If "Cancel", return to Notepad. If "No", ask for another File Name
		...
		do while li_ret = 2				// User clicked "No"
			li_ret = GetFileSaveName ("保存", ls_save_pathname, &
									ls_save_filename,"txt", &
				"テキスト ファイル (*.txt),*.txt,INI Files (*.ini),*.ini,バッチ ファイル (*.bat),*.bat")
...
if GetFileSaveName ("ファイルの削除", ls_fullname, ls_filename, &
						"txt", "テキスト ファイル (*.txt),*.txt,INI Files (*.ini)," + &
						"*.ini,バッチ ファイル (*.bat),*.bat") < 1 then return

if MessageBox ("PowerBuilt メモ帳", &
						ls_fullname + " を削除しますか?", question!, yesno!) = 1 then
...
//////////////////////////////////////////////////////////////////////
PrintDefineFont (li_job, 1, "MS UI Gothic", -9, 400, Default!, Decorative!, &
						false, false)
PrintDefineFont (li_job, 2, "MS UI Gothic", -9, 700, Default!, Decorative!, &
						false, false)
...
//////////////////////////////////////////////////////////////////////
li_pagenum = 1
Print (li_job, "ファイル: " + is_fullname, 6700)
Print (li_job, "ページ " + String (li_pagenum))
Print (li_job, "")
Print (li_job, "")
...
for li_lines = 1 TO LineCount (mle_notepad) 
	...
	li_newpos = Send(Handle (mle_notepad), 187, li_lines - 1, 0)
	Send (Handle (mle_notepad), li_lineindex, li_newpos, li_newpos )
	Print (li_job, mle_notepad.TextLine())
	li_count = li_count + 1
NEXT
...
//////////////////////////////////////////////////////////////////////
if istr_parms.find_loc = 0 then
	MessageBox ("検索", "文字列" + istr_parms.find_string + " が見つかりません", StopSign!)
	Disable (m_notepad.m_search.m_findnexttext)
else
...
elseif istr_parms.replace_event = "search_and_verify" then
	...
	do while li_loc <> 0
		...
		if li_loc <> 0 then
			...
			li_ret = Messagebox ("テキストの置換", &
								istr_parms.find_string + " を " + &
								istr_parms.replace_string + " に置き換えますか?", &
...
choose case String (message.longparm, "address")
	...
	case "undo"
		if CanUndo (smle_notepad) then
			...
			Messagebox ("エラー", "これ以上元に戻すことはできません")
		end if
	...
	case else
		MessageBox ("エラー", "指定された編集処理はありません。")
end choose
...
is_filename = "文書" + String (parent_win.i_document_number)
is_fullname = ""
